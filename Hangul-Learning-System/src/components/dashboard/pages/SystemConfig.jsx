...
const SystemConfig = () => {
  ...
  const renderEditableCell = (record) => {
    ...
    switch (type) {
      case 'number':
        return <InputNumber style={{ width: '100%' }} value={value} onChange={val => handleValueChange(record.key, record.dataType, val)} />;
      case 'switch':
        return <Switch checked={!!value} onChange={val => handleValueChange(record.key, record.dataType, val)} />;
      case 'password':
        return <Input.Password style={{ width: '100%' }} value={value} onChange={e => handleValueChange(record.key, record.dataType, e.target.value)} />;
      default:
        return <Input style={{ width: '100%' }} value={value} onChange={e => handleValueChange(record.key, record.dataType, e.target.value)} />;
    }
  };
